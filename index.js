...
import { geoPlaneGeometry, geoContourGeomtry } from "./index2";
...
function buildSphere() {
  ...
  Object.entries(worldTriJson).forEach(([name, tri]) => {
    ...
    const geometry = geoPlaneGeometry(tri, radius, thickness, 3, {
      top: true,
      bottom: false,
      side: false
    });
    ...
    const planeSide = new THREE.Mesh(
      geoPlaneGeometry(tri, radius, thickness, 3, {
        top: false,
        bottom: false,
        side: true
      }),
...
function buildPlane() {
  ...
  Object.entries(worldTriJson).forEach(([name, tri]) => {
    ...
    const geometry = geoPlaneGeometry(tri, radius, thickness, 2, {
      top: true,
      bottom: false,
      side: false
    });
